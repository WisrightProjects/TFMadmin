...
import { COMPANY_URL } from "core/services/apiURL.service";
...
const CompanyList = () => {
  const [pageFor] = useState("Company List");
  ...
  const createData = async (values) => {
    const resp = await masterService.create(COMPANY_URL, values);
  ...
  const updateData = async (id, values) => {
    const resp = await masterService.update(COMPANY_URL + "/" + id, values);
    if (resp && resp.meta.code === 200) {
  ...
  const loadDataById = async (id) => {
    const resp = await masterService.getById(COMPANY_URL + "/" + id);
    if (resp && resp.meta.code === 200) {
      return resp.data;
    }
    return false;
  ...
  const handleDelete = async (id) => {
    const resp = await masterService.delete(COMPANY_URL + "/" + id);
    if (resp && resp.meta.code === 200) {
  ...
  useEffect(() => {
    const loadList = async (filter) => {
      setSpinner(true);
      const listUrl = COMPANY_URL + "/filter";
