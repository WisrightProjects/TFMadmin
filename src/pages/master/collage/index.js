...
const validationSchema = Yup.object().shape({
  name: Yup.string().label("Collage name").required(),
});

const Collages = () => {
  const [pageFor] = useState("Collages");
  ...
  const createData = async (values) => {
    const resp = await masterService.create(COLLAGE_URL, values);
  ...
  const updateData = async (id, values) => {
    const resp = await masterService.update(COLLAGE_URL + "/" + id, values);
    if (resp && resp.meta.code === 200) {
  ...
  const loadDataById = async (id) => {
    const resp = await masterService.getById(COLLAGE_URL + "/" + id);
    if (resp && resp.meta.code === 200) {
      return resp.data;
    }
    return false;
  ...
  const handleDelete = async (id) => {
    const resp = await masterService.delete(COLLAGE_URL + "/" + id);
    if (resp && resp.meta.code === 200) {
  ...
  useEffect(() => {
    const loadList = async (filter) => {
      setSpinner(true);
      const listUrl = COLLAGE_URL + "/filter";
    ...
    <div>
      ...
      {modalShow && (
        <ModalCommon
          ...
          size="md"
          modalTitle={modalTitle + " College"}
        >
          {modalTitle && (modalTitle === "Add" || modalTitle === "Edit") && (
            <Form onSubmit={handleSubmit(onSubmit)} className="pt-3">
              <Form.Label>College Name <Required/></Form.Label>
