...
const validationSchema = Yup.object().shape({
  name: Yup.string().label("Profession").required(),
  professionCategory: Yup.string().label("Profession category").required(),
});

const ProfessionList = (props) => {
  const { commonData } = props;

  const [pageFor] = useState("Profession List");
  ...
  const columns = [
    ...
    },
    {
      ...
      selector: (row) =>
        getProfessionCategory("professionAs", row?.professionCategory),
    },
    {
      name: "Last Updated",
  ...
  const getProfessionCategory = (key, value) => {
  ...
  const createData = async (values) => {
    const resp = await masterService.create(PROFESSION_URL, values);
  ...
  const updateData = async (id, values) => {
    const resp = await masterService.update(PROFESSION_URL + "/" + id, values);
    if (resp && resp.meta.code === 200) {
  ...
  const loadDataById = async (id) => {
    const resp = await masterService.getById(PROFESSION_URL + "/" + id);
    if (resp && resp.meta.code === 200) {
      return resp.data;
    }
    return false;
  ...
  const handleEdit = async (id) => {
    ...
    if (resp) {
      const professionCategory = commonData?.professionAs?.find(
        (ele) => ele.code === resp?.professionCategory
      );
      setModalTitle("Edit");
      toggleModal();
      setData(resp);
      setValue("name", resp.name, { shouldValidate: true });
      setValue("professionCategory", professionCategory?.code, {
        shouldValidate: true,
      });
    }
  };
  ...
  const handleDelete = async (id) => {
    const resp = await masterService.delete(PROFESSION_URL + "/" + id);
    if (resp && resp.meta.code === 200) {
  ...
  useEffect(() => {
    const handleChange = async (filter) => {
      setSpinner(true);
      const listUrl = PROFESSION_URL + "/filter";
    ...
    <div>
      ...
      {modalShow && (
        <ModalCommon
          ...
          size="md"
          modalTitle={modalTitle + " Profession"}
        >
            ...
            <Form onSubmit={handleSubmit(onSubmit)} className="pt-3">
              ...
              <Form.Group className="search-field form-group">
                ...
                >
                  ...
                  {commonData?.professionAs?.map((ele, ind) => (
...
export default connect(mapStateToProps, null)(ProfessionList);
