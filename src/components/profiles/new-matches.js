...
const NewMatches = (props) => {
  ...
  useEffect(() => {
    if (filter || profileId) {
      ...
      setApiLoad(true);
      setNewMatches([]);
      const getNewMatches = async (filter) => {
        localStorage.setProfileID(profileId);
        const resp = await usersService.newMatches(filter);
    ...
    <Fragment>
      <Row className="d-flex justify-content-center">
        <Col xl={8}>
          ...
          {apiLoad || newMatches !== null && totalPage > 1 && (
            <Pagination
...
export default connect(mapStateToProps, null)(NewMatches);
