...
const PremiumMembers = (props) => {
  ...
  useEffect(() => {
    if (filter || profileId) {
      const getMyMatches = async (filter) => {
        if (apiLoad === true) return;
        setApiLoad(true);
        setNewMatches([]);
        const resp = await usersService.premiumMembers(filter);
...
export default connect(mapStateToProps, null)(PremiumMembers);
