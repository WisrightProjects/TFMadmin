...
const RecentViewedByMe = (props) => {
  ...
  useEffect(() => {
    if (filter || profileId) {
      ...
      setApiLoad(true);
      setRecentViewed([]);
      const getRecentViewed = async (filter) => {
        localStorage.setProfileID(profileId);
        const resp = await usersService.recentViewedByMe(filter);
...
export default connect(mapStateToProps, null)(RecentViewedByMe);
